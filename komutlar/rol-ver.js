...
exports.run = (client, message, args) => {
  ...
  if (message.mentions.users.size < 1) return message.channel.send('❎ | Bu komutun kullanımı şu şekildedir > **d!rol-ver @kullanıcı <rolün adı>**').catch(console.error);
  let user = message.mentions.users.first();
  let guild = message.guild
  let yetki = args.slice(1).join(' ');
  let modlog = guild.channels.find('name', 'darq-bot');
  if (!modlog) return message.channel.send('❎ | **darq-bot** kanalı bulunamadı. Yüklemek İçin d!kur').catch(console.error);
